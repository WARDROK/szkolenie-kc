...
import { MapPin, CheckCircle2, Clock, ChevronRight, Loader2 } from 'lucide-react';
        ...
        <div className="space-y-3">
          {tasks.map((task, i) => (
            ...
            >
              <Link
                to={`/task/${task._id}`}
                className={`block glass rounded-2xl p-4 ${statusColors[task.status]} transition-all active:scale-[0.98]`}
              >
                <div className="flex items-start gap-3">
                  ...
                  <ChevronRight className="text-gray-600 flex-shrink-0 mt-2" size={18} />
                </div>
              </Link>
            </motion.div>
          ))}
